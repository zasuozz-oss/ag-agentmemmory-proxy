...
function browserShimDir(): string | null {
  if (process.env.AGY_NO_BROWSER_SHIM === '1') return null;
  if (cachedShimDir !== null) return cachedShimDir || null;
  cachedShimDir = '';
  try {
...
function shimmedEnv(): NodeJS.ProcessEnv {
...
export async function runWithRetry<T>(
  ...
  for (let i = 1; i <= attempts; i++) {
    ...
    if (i < attempts && backoffMs) await delay(backoffMs);
  }
...
export async function runAgyPrompt(prompt: string, options: AgyRunOptions = {}): Promise<string> {
  ...
  try {
    ...
    return await runWithRetry(
      () => runAgyPromptNow(prompt, { ...options, timeoutMs: attemptTimeout }),
      (out) => out.trim() === '',
      MAX_ATTEMPTS,
      500,
    );
