...
async function isAgyMode(): Promise<{ ok: boolean; values: Record<string, string> }> {
  const values = await readEnvValues();
  return {
...
async function checkClaudeCodeHooks(): Promise<VerificationCheck> {
  ...
  const issues: string[] = [];
  if (!pluginEnabled) issues.push('agentmemory@agentmemory plugin not enabled in settings.json');
  if (!hasAgentmemoryUrl) issues.push('AGENTMEMORY_URL not set in settings.json env (MCP template vars will be unresolved)');
...
function checkAgyProxy(): VerificationCheck {
...
function checkAgyCli(values: Record<string, string>): VerificationCheck {
...
function checkViewer(): VerificationCheck {
