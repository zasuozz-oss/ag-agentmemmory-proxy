...
export const LOCAL_AGENTMEMORY_ENV_VALUES = {
...
const LLM_KEYS = Object.values(PROVIDER_CONFIG).flatMap((config) => [config.key, config.modelKey, config.baseUrlKey].filter((key) => Boolean(key)));
...
export function detectActiveApiProvider(values) {
    for (const [provider, config] of Object.entries(PROVIDER_CONFIG)) {
        if (values[config.key]?.trim())
            return provider;
    }
    return undefined;
}
...
export function agentmemoryEnvPath(home = os.homedir()) {
    return path.join(home, '.agentmemory', '.env');
}
export function buildManagedEnvValues(options = {}) {
...
export async function upsertAgentmemoryEnv(filePath = agentmemoryEnvPath(), options = {}) {
...
export async function readEnvValues(filePath = agentmemoryEnvPath()) {
    const current = await fs.readFile(filePath, 'utf8').catch(() => '');
